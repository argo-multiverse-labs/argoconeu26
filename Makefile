...
.PHONY: clean
clean: rm-kind ## Clean up the KinD cluster and remove tenant-c
	rm -rf tenants/tenant-c && git add -A tenants/ && git commit -m "reset: remove tenant-c" && git push

.PHONY: git-clean
git-clean: ## remove tenant-c
	rm -rf tenants/tenant-c && git add -A tenants/ && git commit -m "reset: remove tenant-c" && git push
